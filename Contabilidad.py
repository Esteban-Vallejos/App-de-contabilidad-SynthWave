...
root = Tk()
root.title('Contabilidad | SynthWave')
root.resizable(0,0)
try:
...
maintitlelabel = ttk.Label(root, text="App de Contabilidad", foreground="#2C3639",style='title.TLabel')
maintitlelabel.pack()
subtitlelabel = ttk.Label(root, text="_____________________________________________________", foreground="#3F4E4F",style='subtitle.TLabel')
subtitlelabel.pack()
...
def company_data():
    ...
    def edit_data():
        if company_name_entry.get() == "" or address_entry.get() == "" or tel_entry.get() == "": 
            ...
            # Cree una base de datos o conéctese a una que existe
    ...
    #cerrar sin función de guardar
    def close_window():
      # Cree una base de datos o conéctese a una que exista
        conn = sqlite3.connect(db_name)
    ...
    #Validación de casillas de entrada
    def only_text(inp):
        if len(inp) > 50:
            return False
        elif inp == "":
   ...
   #Administrar cerrar ventana sin guardar
    company_data_frame.protocol("WM_DELETE_WINDOW", close_window)


    #Cajas de entrada y etiqueta
    ...
    #añadir datos a la función de entrada
    def add_data():
        # Cree una base de datos o conéctese a una que exista
        conn = sqlite3.connect(db_name)
...
def concepts():
    ...
    # Configurar los colores de la vista de árbol
    ...
    # Crear un marco de vista de árbol
    ...
    # Dar formato a nuestras columnas
    ...
    # Crear etiquetas de fila rayada
    ...
    #función de mesa limpia
    def clean_table():
        for item in my_tree.get_children():
            my_tree.delete(item)

    #agregar conceptos a la vista de árbol
    def add_conceptos():
        # Cree una base de datos o conéctese a una que exista
        conn = sqlite3.connect(db_name)
    ...
    #Crear entrada de validación de concepto
    def only_text(inp):
        if len(inp) > 40:
            return False
        elif inp == "":
    ...
    #Funciones de búsqueda de entrada de texto temporal
    ...
    #función seleccionada de enfoque
    def selected(e):
        selected_filter = my_tree.focus()
        if selected_filter != "":
            # Grabe el número de registro
            selected = my_tree.focus()
            # grave el registro de la variable/values
    ...
    #Vincular la vista de árbol
    my_tree.bind("<ButtonRelease-1>", selected)


    #cajas de entrada para seleccionar y crear concepto
...
def cuentas_adm():
    ...
    # Crear una barra de desplazamiento de vista de árbol
    tree_scroll = Scrollbar(tree_frame, width=20)
    tree_scroll.pack(side=RIGHT, fill=Y)
    ...
    # Crear etiqueta de fila rayadas
    my_tree.tag_configure('oddrow', background="white")
    ...
    def select_record(e):
        ...
        if selected_filter != "":
            ...
            if filter[0] != "Totales" and selected_filter != "":	 
                #Habilitar de nuevo add_init_button
                ...
                # Configurar id_entry en estado Normal
                saldoActual_entry.config(state="normal")
                # Borrar cuadros de entrada
                idCuenta_entry.delete(0, END)
                codCuenta_entry.delete(0, END)
                ...
                # Configurar id_entry en solo lectura
                saldoActual_entry.config(state="readonly")
	...
	#Funciones de busqueda de entrada de texto temporal
    def temp_text_cod(e):
        search_entry_cod.delete(0, END)
    ...
    #Registre las funciones para validar la entrada	
    ...
    #Crear entrada y botones en el cuadro de búsqueda.
	...
	#Vincular eventos en la entrada de búsqueda
    ...
    #Vincular la vista de árbol para seleccionar
    ...
    class MyDateEntry(DateEntry):
        ...
        def drop_down(self):
            ...
            if self._calendar.winfo_ismapped():
                ...
                # predeterminado: menú desplegable alineado a la izquierda debajo de la entrada
                x = self.winfo_rootx()
                y = self.winfo_rooty() + self.winfo_height()
                if x + w > x_max:  # el menú desplegable sale de la pantalla
                    # alinear a la derecha el menú desplegable
                    x += self.winfo_width() - w
                if y + h > y_max:  # el menú desplegable sale de la pantalla
                    # abajo-alinear el menú desplegable
